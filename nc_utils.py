...
def plot_nc_visualizations(metrics, output_dir="outputs"):
    ...
    plt.title("NC1: Intra-class Variance per Class")
    plt.xlabel("Class")
    plt.ylabel("Variance (Squared L2 Norm)")
    ...
    plt.title(f"NC2: ETF Geometry\n(Theoritical angle = {metrics['theoretical_angle']:.4f})")
    plt.xlabel("Class i")
    plt.ylabel("Class j")
    ...
    plt.axhline(y=1.0, color='r', linestyle='--', label='Perfect alignement (1.0)')
    plt.title("NC3: Cosine Similarity between weights (W) and means (M)")
    plt.xlabel("Class")
    ...
    if 'biases' in metrics:
        ...
        plt.title("NC5:Bias Value of the Classifier ")
        plt.xlabel("Class")
    ...
    print(f"NC visualisations saved to: '{output_dir}/'.")
